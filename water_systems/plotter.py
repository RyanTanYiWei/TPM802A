...
def parallel_plots_many_policies(
    ...
    names_unit_direction = {
        "Egypt Irr. Deficit": ("BCM/year","min"),
        "Egypt 90th Irr. Deficit": ("BCM/month","min"),
        "Egypt Low HAD":("%","min"),
        "Sudan Irr. Deficit": ("BCM/year","min"),
        "Sudan 90th Irr. Deficit": ("BCM/month","min"),
        "Ethiopia Hydropower": ("TWh/year","max"),
    }

    names = list(obj_df.columns)
    units = [(names_unit_direction[i])[0] for i in names]
    directions = [(names_unit_direction[i])[1] for i in names]

    objectives_df = obj_df.copy()
    objectives_df["Egypt Low HAD"] = 100 * (objectives_df["Egypt Low HAD"])
    ...
    plt.legend(
        flip([handles_dict[label] for label in labels], 6),
        flip(labels, 6),
